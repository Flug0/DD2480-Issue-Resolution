...
class FeedExporter:
    ...
    def _import_processor(self, processor_info):
        ...
        try:
            ...
            raise NotConfigured("Missing 'path' key in processor configuration")
    ...
    def transform_item_field(self, proc_name, former_item, new_item):
        ...
        if not proc_name in  self.feeds:
            raise KeyError("Processor not found")
        if self.feeds[proc_name]["fields"] is None:
            raise KeyError("Fields not found")
